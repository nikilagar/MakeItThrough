...
1.  The selected text is sent to a **dedicated cloud backend server**.
2.  The backend server transmits the text to the **Google Gemini API** for analysis.
...
*   **Cloud Backend**: The dedicated backend server logs are used only for processing requests and debugging. No personal data is stored.
